...
if [ "$1" = './TrackmaniaServer' ]; then
	...
	if [ "$MASTER_PASSWORD" ]; then MASTER_PASSWORD=$(echo ${MASTER_PASSWORD} | sed -e 's/\$/\\\$/g' -e 's/`/\\`/g'); configs+=("'/dedicated/masterserver_account/password' -v \"${MASTER_PASSWORD}\""); else echo "[ERROR] Masterserver password not set!"; exit 1; fi
